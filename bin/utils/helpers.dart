...
void createFeatureStructure(String featureName, String folderName) {
  final root = 'lib/features/$folderName';

  if (Directory(root).existsSync()) {
    print('Feature $folderName already exists. Skipping creation.');
    return;
  }
  ...
  final blocFile = File('$root/bloc/${folderName}_bloc.dart');
  blocFile.writeAsStringSync(blocTemplate(featureName, folderName));

  final eventFile = File('$root/bloc/${folderName}_event.dart');
  eventFile.writeAsStringSync(eventTemplate(featureName,folderName));

  final stateFile = File('$root/bloc/${folderName}_state.dart');
  stateFile.writeAsStringSync(stateTemplate(featureName,folderName));

  final pageFile = File('$root/pages/${folderName}_page.dart');
  pageFile.writeAsStringSync(pageTemplate(featureName));

  print('Created default BLoC, Event, State, and Page files for $folderName.');
}

void addRouteToRouter(String featureName, String folderName) {
  ...
  final importStatement =
      "import '../features/$folderName/pages/${folderName}_page.dart';";
  final newRoute = """
      GoRoute(
  ...
  print('Added import and route for $folderName in routes.dart');
}

void addBlocProviderToMain(String featureName, String folderName) {
  ...
  final importStatement =
      "import 'features/$folderName/bloc/${folderName}_bloc.dart';";
